...
dst = cv.warpAffine(img,M,dsize=(cols*10, rows*10),flags=cv.INTER_LINEAR)
...
matrix = cv.getRotationMatrix2D((2, 4), angle=30, scale=1.4)
dst = cv.warpAffine(img, matrix, dsize=(cols, rows), flags=cv.INTER_CUBIC,
...
dst = cv.sepFilter2D(img, ddepth=-1, kernelX=kernelX, kernelY=kernelY,
